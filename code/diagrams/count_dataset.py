...
# # 保存高分辨率 PNG
# plt.savefig("diagram_model_size_distribution_with_labels.pdf", dpi=300, bbox_inches="tight")
# plt.show()
...
import numpy as np
# 使用 numpy linspace 生成紧凑 y 位置
# y_ind = np.linspace(0, len(industry_application) - 1, len(industry_application))
# y_meth = np.linspace(len(industry_application), len(industry_application) + len(methodology_tool) - 0.5, len(methodology_tool))
...
# plt.xticks(rotation=45)
# plt.ylabel("Model Size (#elements)")
# plt.title("Distribution of Model Sizes in the Test Set Across Diagram Types")
# plt.tight_layout()
...
# # 保存高分辨率 PNG
# plt.savefig("diagram_model_size_distribution_with_labels_testdataset.pdf",dpi=300,bbox_inches="tight")
# plt.show()